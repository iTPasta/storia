...
serve(async (req) => {
  ...
  try {
    ...
    if (storyName) {
      ...
      prompt = isEnglish
        ? `Create a children's story titled "${storyName}" suitable for children aged 5 to 7 years old. The story should have around 10 short segments, each with an emotion tag (${emotionsString}). Each segment must be composed of a maximum of 30 words. Format as JSON: { "title": "Story Title", "title_fr": "French Title", "segments": [{"text": "English text", "text_fr": "French text", "emotion": "emotion"}] }. Emotions should match the content of each segment.`
        : `Créez une histoire pour enfants intitulée "${storyName}" adaptée aux enfants âgés de 5 à 7 ans. L'histoire doit avoir environ 10 courts segments, chacun avec une étiquette d'émotion (${emotionsString}). Chaque segment doit être composé d'un maximum de 30 mots. Format JSON: { "title": "Titre en anglais", "title_fr": "Titre en français", "segments": [{"text": "Texte anglais", "text_fr": "Texte français", "emotion": "émotion"}] }. Les émotions doivent correspondre au contenu de chaque segment.`;
    } else {
